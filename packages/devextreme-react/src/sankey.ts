...
type ITooltipProps = React.PropsWithChildren<{
  ...
  cornerRadius?: number;
  customizeLinkTooltip?: ((info: { source: string, target: string, weight: number }) => object);
  customizeNodeTooltip?: ((info: { label: string, title: string, weightIn: number, weightOut: number }) => object);
  enabled?: boolean;
  font?: BaseWidgetTypes.Font;
